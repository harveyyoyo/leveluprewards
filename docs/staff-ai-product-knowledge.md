...
- **Admin** — manage students, classes, teachers, categories, points, prizes, raffles, imports/exports, attendance, and other school configuration. The administration panel features state-of-the-art interactive dashboards and a stunning **Admin Welcome Hero** with real-time stat tiles (total students, class count, active staff, and listed prize count).
  - **Library** — Open **Admin → Library** or `/{schoolId}/library`. Dedicated hub with circulation desk, catalog, loans, kiosk, and Theme & Atmosphere. A school can have **more than one library** (school library and class libraries). Each library has its own catalog, desk, kiosk, and checkout limit. Existing books stay in **School Library** until staff move them. Add or rename libraries under **Library → Settings**. Kiosk stations ask which library they belong to when more than one exists (`?library=` in the URL).
    - **Library AI helper**: Staff see an **Ask** button in the library header and a floating sparkle button (same idea as LevelUp Help and support). It answers library how-to questions and can use live counts (copies on the shelf, checked out, overdue titles only — never student names). The **Guide** button is a written handbook; the sparkle helper is the chat assistant.
    - **Home / front door**: Storybook doors into Librarian, Catalog, and the student station. Home icon in the header returns here.
    - **Librarian desk**: Search a student or title, check books in and out, and see overdue copies. Scan a barcode from most screens to jump here.
    - **Catalog**: Browse and add copies. **Add Book** plus camera or handheld scan fills title, author, cover, and summary. Print barcode stickers and spine labels from selected copies. Filter by shelf, reading level, genre, or availability.
    - **Student station / Kiosk**: Students scan their badge then a book to borrow. They can return and leave star ratings when that is enabled. Dedicated fullscreen kiosk: `/{schoolId}/library/kiosk`. Turn the shared-device station on or off under **Library → Settings**.
    - **Settings (gear)**: Loan length, books per student, renewals, late-return points, camera scanning, self-checkout, multiple libraries, and Theme & Atmosphere.
    - **Reports**: Opened from the Librarian page (not a top tab). Overdue lists, popular titles, and printable exports.
    - **Shelf audit**: Walk the aisles scanning copies to find missing or misplaced books.
