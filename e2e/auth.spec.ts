...
test('login and navigate to portal and student pages', async ({ page }) => {
    ...
    await studentPortalBtn.waitFor({ state: 'visible', timeout: 30000 });
    await studentPortalBtn.click();

    // Verify it goes to student and stays there
    console.log("Waiting for student page...");
    try {
        await page.waitForURL(url => url.pathname.endsWith('/student'), { timeout: 30000 });
        await page.waitForSelector('text=System Ready', { timeout: 30000 });
    } catch (e) {
