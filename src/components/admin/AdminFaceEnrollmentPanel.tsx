...
import { checkFaceEnrollmentDuplicate, notifyFaceEnrollmentChanged } from '@/lib/faceEnrollment';
...
export function AdminFaceEnrollmentPanel({
  ...
  const handleTrain = useCallback(async () => {
    ...
    try {
      ...
      const descriptorPayload = descriptor.map((n: number) => Number(n));
      const duplicateCheck = await checkFaceEnrollmentDuplicate(
        schoolId,
        studentId,
        descriptorPayload,
        functions,
      );
      if (duplicateCheck?.duplicate && duplicateCheck.matchedStudentId) {
        const pct = Math.round(Math.max(0, Math.min(1, duplicateCheck.confidence ?? 0)) * 100);
        const targetLabel = studentLabel || `student ${studentId}`;
        playSound('error');
        const proceed = await confirm({
          title: 'This face is already enrolled',
          description: `This face closely matches student ID ${duplicateCheck.matchedStudentId} (${pct}% similar). Training it for ${targetLabel} too can cause unclear match at sign-in. Use the correct student in front of the camera, or remove the other enrollment first.`,
      ...
      await enroll({
        ...
        allowDuplicate: duplicateCheck?.duplicate === true,
      });
      playSound('success');
      ...
      const details = e?.details as { matchedStudentId?: string; confidence?: number } | undefined;
      const matchedStudentId =
        typeof details?.matchedStudentId === 'string' ? details.matchedStudentId : undefined;
      if (matchedStudentId) {
        const pct =
          typeof details?.confidence === 'number'
            ? Math.round(Math.max(0, Math.min(1, details.confidence)) * 100)
            : null;
        playSound('error');
        toast({
          variant: 'destructive',
          title: 'This face is already enrolled',
          description: `This face matches student ID ${matchedStudentId}${pct !== null ? ` (${pct}% similar)` : ''}. Remove that enrollment first or train the correct student.`,
        });
        setStatus(null);
        return;
      }
