...
export type HallOfFameViewProps = {
...
export default function HallOfFamePage({
    ...
    }, [isPreview, isInitialized, loginState, router, schoolId, toast]);
    ...
    const isPortrait = isPreview ? previewLayout === 'portrait' : layout === 'portrait';
    ...
    const pageContent = (
        <div
          className={cn(
            ...
            isPreview && "relative overflow-hidden bg-background",
            isFullscreen
              ? "fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-background"
              : !isPreview && "relative min-h-screen flex flex-col items-center",
            !isFullscreen && !isPreview && animBackdrop ? "bg-transparent" : !isPreview ? "bg-background" : undefined,
          )}
          style={themeStyle}
        >
            {!isFullscreen && !isPreview && showHallLocalDecor && (
            ...
            <div
              className={cn(
                ...
                isPreview && 'h-full w-full',
                !isFullscreen && !isPreview && 'w-full min-h-screen',
                !isFullscreen && !isPreview && (isPortrait ? 'max-w-md mx-auto' : 'max-w-full'),
              )}
              style={isFullscreen ? getHallOfFameStageSizeStyle(isPortrait) : undefined}
            >
            <div
              className={cn(
                ...
                isFullscreen || isPreview ? "px-3 pt-2 pb-2" : "px-4 sm:px-8 pt-8 md:pt-12",
                !isFullscreen && !isPreview && (isCompactDisplayMode(settings.displayMode) ? 'pb-24' : 'pb-12'),
              )}
            >
                <div className={cn(
                  "z-20 shrink-0 bg-background/85 backdrop-blur-md border-b border-border/40",
                  isFullscreen || isPreview ? "mb-2 py-1.5" : "sticky top-0 mb-8 py-3",
                )}>
    ...
    if (isFullscreen && portalReady && !isPreview) {
