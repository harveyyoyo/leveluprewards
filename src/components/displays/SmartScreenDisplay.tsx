...
export function SmartScreenDisplay({
  ...
  const compact = isPortrait || isDashboard || isPreview;
    ...
    >
      <div
        className={cn(
          ...
          isPreview
            ? 'h-full grid-rows-[minmax(120px,0.45fr)_minmax(0,1.55fr)] gap-2'
            : isPortrait
              ...
              {schoolMeta?.logoUrl ? (
                <img
                  ...
                  className={cn(
                    ...
                    isPortrait || isPreview ? 'h-9 w-9' : 'h-12 w-12',
                  )}
                />
              ) : (
                <div
                ...
                >
                  <School className={isPortrait || isPreview ? 'h-5 w-5' : 'h-6 w-6'} aria-hidden />
                </div>
              )}
              <div className="min-w-0">
                <p
                  className={cn(
                    ...
                    isPortrait || isPreview ? 'text-[10px]' : 'text-sm',
                    theme.quiet,
                  )}
                >
                  {schoolName}
                </p>
                <h1 className={cn('truncate font-black', isPortrait || isPreview ? 'text-lg' : 'text-3xl')}>{title}</h1>
                {screenProfileName ? (
                  <p
                    className={cn(
                      'truncate font-bold uppercase tracking-[0.18em]',
                      isPortrait || isPreview ? 'text-[9px]' : 'text-[10px]',
                      theme.quiet,
                    )}
                  >
          ...
          <div className="flex min-h-0 flex-1 flex-col justify-center">
            <p
              className={cn(
                ...
                isPortrait || isPreview ? 'mt-2 text-[10px]' : 'mb-3 mt-4 text-sm',
                theme.accent,
              )}
            >
              {timeGreeting(currentHour)}
            </p>
            <p className={cn('font-black leading-none', isPortrait || isPreview ? 'text-4xl' : compact ? 'text-6xl' : 'text-8xl')}>
              {formatSmartScreenTime(now, displayTimeZone)}
            </p>
            <p className={cn('mt-2 font-bold', isPortrait || isPreview ? 'text-sm' : 'text-2xl', theme.quiet)}>
              {formatSmartScreenDate(now, displayTimeZone)}
            </p>
            {showHebrewDate ? (
              <p
                className={cn(
                  'mt-1 font-black text-amber-700 dark:text-amber-200',
                  isPortrait || isPreview ? 'text-sm' : 'text-xl',
                )}
                dir="rtl"
                lang="he"
              >
            ...
            <p
              className={cn(
                'mt-3 max-w-4xl font-black leading-tight',
