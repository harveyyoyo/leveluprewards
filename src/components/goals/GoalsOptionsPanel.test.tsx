...
it('retains every setting and changes only the selected option', () => {
  ...
  render(<GoalsOptionsPanel value={{ ...DEFAULT_GOALS_OPTIONS, hallwaySpotlight: false }} onChange={onChange} />);
  ...
  expect(onChange).toHaveBeenCalledWith({ ...DEFAULT_GOALS_OPTIONS, hallwaySpotlight: false, celebrateOnAward: false });
});
