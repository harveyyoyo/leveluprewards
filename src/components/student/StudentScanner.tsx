...
import { findLibraryItemByUpc } from '@/lib/library/libraryOperations';
...
import {
    persistKioskLoginTab,
    resolveKioskLoginTab,
...
export function StudentScanner({
    ...
    const [loginTab, setLoginTabState] = useState<KioskLoginTabId>('nfc');
    const loginTabHydratedRef = useRef(false);
    const loginTabSchoolRef = useRef(schoolId);

    const setLoginTab = useCallback(
        (tab: string) => {
            const next =
                tab === 'nfc' || tab === 'manual' || tab === 'camera' || tab === 'face' ? tab : 'nfc';
            setLoginTabState(next);
            persistKioskLoginTab(schoolId, next);
            if (next !== 'camera') {
                clearKioskCameraLoginTabPref(schoolId);
            }
        },
        [schoolId],
    );
    ...
    useEffect(() => {
        if (loginTabSchoolRef.current !== schoolId) {
            loginTabSchoolRef.current = schoolId;
            loginTabHydratedRef.current = false;
        }
        if (!loginTabHydratedRef.current) {
            loginTabHydratedRef.current = true;
            setLoginTabState(resolveKioskLoginTab(schoolId, availableLoginTabs, 'nfc'));
            return;
        }
        setLoginTabState((current) => {
            if (availableLoginTabs.includes(current)) return current;
            return resolveKioskLoginTab(schoolId, availableLoginTabs, 'nfc');
        });
    ...
    const startFaceCamera = useCallback(async (): Promise<HTMLVideoElement | null> => {
        ...
        try {
            ...
            applyFaceCameraPreviewStyle(video);
            return video;
        } catch (e: any) {
            const denied =
    ...
    const { videoRef, hasCameraPermission: hookHasPermission, zoom, setZoom, scanStatus } = useBarcodeScanner(
        cameraScanSettingOn,
    ...
    }, [qrEnabled, loginTab, cardEnabled, typeEnabled, setLoginTab, schoolId]);
    ...
    return (
        ...
        <div className={cn(
            ...
            <div className="p-3 [@media(max-height:720px)]:p-2">
                <Tabs className="w-full" value={loginTab} onValueChange={setLoginTab}>
                    ...
                    <TabsContent value="camera">
                        <div className="py-2 [@media(max-height:720px)]:space-y-2">
