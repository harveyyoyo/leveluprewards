...
export type ThemeTweakSettings = {
  active: number;
  headingFont: string;
  bodyFont: string;
  hue: number;
...
export function ClassroomThemeChooserModal({
  open,
  onOpenChange,
  currentThemeSlug,
  onApplyTheme,
  ...
  onApplyTheme: (slug: string, settings: ThemeTweakSettings) => void;
}) {
  const initialIndex = useMemo(() => {
    if (!currentThemeSlug) return 0;
    const found = CLASSROOM_THEME_DESIGNS.findIndex((d) => d.slug === currentThemeSlug);
    return found >= 0 ? found : 0;
  }, [currentThemeSlug]);

  const [settings, setSettings] = useState<ThemeTweakSettings>(() => ({
    ...DEFAULT_SETTINGS,
    active: initialIndex,
  }));
      ...
      try {
        ...
        if (saved) {
          ...
          setSettings((prev) => ({
            ...prev,
            ...parsed,
            active: initialIndex >= 0 ? initialIndex : parsed.active ?? 0,
          }));
        }
      } catch {
        // storage ignored
  ...
  const handleApply = () => {
    onApplyTheme(activeDesign.slug, settings);
    onOpenChange(false);
  };
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Classroom Theme Chooser"
      className="fixed inset-0 z-[1000] flex flex-col bg-zinc-950 text-zinc-100 font-sans animate-in fade-in duration-200"
    >
      {/* Top Header */}
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-zinc-800 bg-zinc-900/90 px-3 sm:px-5 backdrop-blur-sm z-30">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            ...
            aria-label="Close"
            className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors ml-1"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
