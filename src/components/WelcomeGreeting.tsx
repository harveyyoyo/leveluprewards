...
interface WelcomeGreetingProps {
  name?: string;
  /** Firestore / admin default style id. */
  initialStyleId?: string;
  /** When set, chosen style is saved under this key in localStorage (kiosk). */
  persistStyleStorageKey?: string | null;
  /** When true, only the greeting card is shown (no style / palette pickers). */
...
export function WelcomeGreeting({
  ...
  useEffect(() => {
    if (!persistStyleStorageKey || typeof window === "undefined") return;
    try {
      localStorage.setItem(persistStyleStorageKey, styleId);
    } catch {
      // ignore quota / private mode
    }
  }, [persistStyleStorageKey, styleId]);
...
export type WelcomeGreetingStyleId = StyleId;
export const WELCOME_GREETING_STYLES = STYLES;
