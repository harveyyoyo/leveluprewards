...
function officeProductKnowledge(): string {
  ...
  return [
    ...
    '- **Customize** (top of the left menu, under the school name) is personal to this device: a **Color theme** (Teal, Blue, Purple, Rose, Green, Orange or Gray — changes the menu and button colors), a wide layout switch, and which sections show in their own menu.',
    ...
    '- **Sign out**: click your name at the bottom of the left menu, then **Sign out**.',
    '- Nothing is ever erased: anything removed stays in Reports → Change history, with who did it and when.',
    '',
    '**Not in the School Office**: there is no logo or branding setting and no dark mode switch; colors are only the per-device Color theme in Customize. If asked about something it does not have, say so plainly.',
    '',
