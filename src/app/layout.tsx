    ...
    >
      <head>
        ...
        {process.env.NODE_ENV === 'development' ? (
            <script
              dangerouslySetInnerHTML={{
                __html: `(function(){if(!('serviceWorker' in navigator))return;navigator.serviceWorker.getRegistrations().then(function(r){return Promise.all(r.map(function(x){return x.unregister()}))});if('caches' in window){caches.keys().then(function(k){return Promise.all(k.filter(function(n){return n.indexOf('levelup-')===0}).map(function(n){return caches.delete(n)}))})}})();`,
              }}
            />
        ) : null}
      </head>
