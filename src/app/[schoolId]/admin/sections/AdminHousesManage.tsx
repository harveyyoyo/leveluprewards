          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 rounded-2xl border bg-muted/20 p-4 md:p-5">
              ...
              <div className="space-y-3 border-t border-border/50 pt-4">
                <div>
                  <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
                    Sorting ceremony — reveal effect
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Same celebration styles as classroom points (flash, confetti, fireworks, and more).
                  </p>
                </div>
                <RadioGroup
                  value={resolveHouseSortingCelebrationEffect(settings.houseSortingCelebrationEffect)}
                  onValueChange={(v) =>
                    updateSettings({ houseSortingCelebrationEffect: v as ClassroomCelebrationEffect })
                  }
                  className="grid grid-cols-2 gap-2 sm:grid-cols-4"
                >
                  {HOUSE_SORTING_CELEBRATION_OPTIONS.map((key) => (
                    <label key={key} className="flex cursor-pointer items-center gap-2 rounded-lg border border-border/60 px-2.5 py-2">
                      <RadioGroupItem value={key} aria-label={HOUSE_SORTING_CELEBRATION_LABELS[key]} />
                      <span className="text-xs font-medium">{HOUSE_SORTING_CELEBRATION_LABELS[key]}</span>
                    </label>
                  ))}
                </RadioGroup>
              </div>
              <div className="flex items-center justify-between gap-3 border-t border-border/50 pt-4">
                <div>
                  <Label className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
                    Sorting ceremony — house fly-up
                  </Label>
                  <p className="text-xs text-muted-foreground mt-1">
                    Kiosk-style fly-up with the house name when a student is revealed (like classroom +PTS).
                  </p>
                </div>
                <Switch
                  checked={settings.houseSortingShowFlyUp !== false}
                  onCheckedChange={(v) => updateSettings({ houseSortingShowFlyUp: v })}
                  aria-label="Sorting ceremony house fly-up"
                />
              </div>
              <div className="flex items-center justify-between gap-3 border-t border-border/50 pt-4">
                <div>
