...
import { Copy, Download, ExternalLink, GraduationCap, Lock, MonitorOff, Unlock, Users } from 'lucide-react';
...
export function AdminFamilyPortalsTab({ schoolId, students }: Props) {
  ...
  const studentPortalOn = settings.enableStudentPortal === true;
  ...
  const handleCopyUrl = async (url: string, label: string) => {
    try {
      ...
      toast({ title: 'Link copied', description: `Share this URL for ${label}.` });
    } catch {
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <Helper content="Families sign in with the parent email on file for each student. They only see notes teachers marked as shared with parents, plus points and attendance.">
        ...
        <CardContent className="space-y-6">
          {!classroomOn ? (
            <Alert>
              <AlertTitle>Classroom Management is off</AlertTitle>
              <AlertDescription>
                Turn on Classroom Management before enabling the parent portal. Note sharing checkboxes for
                teachers live under Admin → Classroom → Note sharing.
              </AlertDescription>
            </Alert>
          ) : null}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-xl border p-4">
            <div>
              <Label htmlFor="enable-parent-portal-tab" className="font-semibold">
                Enable parent portal
              </Label>
              <p className="text-sm text-muted-foreground mt-1">
                When on, families can sign in at the parent URL below. The main school portal page does not
                list this link — share the URL directly with families.
              </p>
            </div>
            <Switch
              id="enable-parent-portal-tab"
              checked={parentPortalOn}
              disabled={!classroomOn}
              onCheckedChange={(checked) => updateSettings({ enableParentView: checked })}
            />
          </div>
          ...
          {parentPortalOn ? (
            <>
              <div className="rounded-xl border bg-muted/20 p-4 text-sm text-muted-foreground space-y-2">
                <p className="font-semibold text-foreground">How notes reach parents</p>
                <p>
                  Teachers choose per note in Class Awards Live whether to share with parents (Admin →
                  Classroom → Note sharing must allow it). Only notes with &ldquo;Share with parent&rdquo;
                  checked appear in the parent portal.
                </p>
              </div>

              <div className="space-y-2">
                <Label>Parent portal URL</Label>
                <div className="flex flex-wrap gap-2">
                  <Input readOnly value={parentPortalUrl} className="font-mono text-sm flex-1 min-w-[200px]" />
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => void handleCopyUrl(parentPortalUrl, 'parent sign-in')}
                  >
                    <Copy className="mr-2 h-4 w-4" aria-hidden />
                    Copy
                  </Button>
                  <Button type="button" variant="outline" asChild>
                    <a href={parentPortalUrl} target="_blank" rel="noopener noreferrer">
          ...
          {studentPortalOn ? (
            <>
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  <Input readOnly value={studentPortalUrl} className="font-mono text-sm flex-1 min-w-[200px]" />
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => void handleCopyUrl(studentPortalUrl, 'student home access')}
                  >
                    <Copy className="mr-2 h-4 w-4" aria-hidden />
                    Copy
                  </Button>
...
/** @deprecated Use AdminFamilyPortalsTab */
export const AdminStudentPortalTab = AdminFamilyPortalsTab;
